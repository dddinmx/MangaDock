...
def register_routes(bp):
    ...
    @bp.get('/')
    def api_v1_root():
        # Unauthenticated callers only see a minimal stub (no route map / auth
        # details / app version). Authenticated clients get the full catalog.
        user, _error = get_api_authenticated_user()
        if not user:
            return api_ok({
                'name': 'MangaDock API',
                'version': 'v1',
