...
@csrf.exempt
def api_auth_login():
    # Match /api/v1/auth/login: session/cookie clients must present CSRF.
    # Basic Auth clients skip CSRF via require_csrf_for_session_auth.
...
@app.route('/api/info')
def api_info():
    # Public discovery payload for clients (e.g. Tachiyomi). Keep minimal:
    # no app build version, no full route map / auth implementation details.
    return jsonify({
        'name': 'MangaDock',
        'version': '1',
