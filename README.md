...
- **登录页封面成人内容过滤加固**：此前登录页封面墙只按 `comic.json` 里的 18+ 来源判定，而候选池中有相当比例的封面**根本没有源映射**，判定对它们完全失效 —— 成人向封面会出现在未登录即可访问的登录页上。现在改为三层过滤：18+ 来源判定 + 人工排除名单 + 名字硬关键词兜底。过滤只作用于登录页抽样，**漫画库、书架与客户端下发的封面一律不受影响**。
- **登录页补位封面清理**：随仓库分发、给全新部署用户打底的 24 张漫画补位封面中，5 张画风不适宜公开页面，已替换为无争议封面。
...
- **首页书轨入口修正**：「最近观看」书轨的「查看全部」改回指向全部书架（此前指向历史页导致书架没有入口）。

## v2.10.2

- **底部 Dock 栏开关**：「设置 → 外观」新增「底部 Dock 栏」分段开关（默认关闭），手机端可自由显隐底部导航胶囊，偏好按设备记忆。
- **首页书轨改「最近观看」**：首页底部横滑书轨由「最近入库」改为「最近观看」，按观看时间倒序展示最近看过的漫画，入口直达历史页。
...
- **v2.10.1 容器修复**：容器内 Gunicorn 绑定 `0.0.0.0`（此前端口映射不可达）；出站直连建连改用 urllib3 `create_connection`（修复 `socket_options` TypeError 导致所有下载任务失败）。

## Docker 部署

Docker Hub 镜像：[`dddinmx/mangadock:v2.11.1`](https://hub.docker.com/r/dddinmx/mangadock/tags)。`v2.11.1` 和 `latest` 同时提供 `linux/amd64` 与 `linux/arm64`。
