...
- **下载页措辞统一为「整本下载」**：设置页、更新页与下载页口径一致；格式选择（CBZ / PDF）明确标注只作用于漫画，番茄小说固定输出 EPUB。
- **上游网络层重试**：抓取元数据/HTML 遇到瞬时 `ConnectionReset`、`SSLEOFError`、超时不再直接报废整个任务，改为指数退避重试；DNS 解析抖动同样重试。图片字节下载保持单次重试，避免与外层重试相乘。
- **后台 worker 守护线程**：下载 / 命令 worker 意外退出会被主进程自动补员；若任务 worker 全部离线，残留的「运行中」任务会自动退回队列，不再出现「任务永远卡在等待中、只能重启服务」。
- **封面获取修复**：封面尚未落盘时改为返回 `404`（不再回落占位图 + `200`），客户端因此不会把占位图当成有效封面写进图片缓存；API 下发的封面地址改用 Web 端同款静态路径，响应不再带 `Vary: Cookie`，带磁盘缓存的客户端图片缓存可正常命中。
- **扩展仓库更新到 v1.6.5**：修复 Tachimanga 在线封面整架不显示的问题（根因是扩展侧 OkHttpClient 的派生方式，未共享客户端的 Cache / CookieJar）。
- **新增 `tools/repair_cover_images.py`**：封面体检与修复工具，找出无法解码、或真实格式与扩展名错配（如 WebP 顶着 `.jpg`）的封面文件，Safari/WebKit 下这类文件会直接解码失败。

## v2.8.0

- **新增嬉皮漫畫（`hipmh.com`）来源**：支持作品页链接整本下载与后续更新检查，下载页提供站点入口，域名已加入服务端出网白名单。
- **漫画书架重做，与小说书架同构**：`/comics` 改为单层平铺封面网格 + 分页，不再按分组切成带横滑轨的分段区块；总览页卡片下方会显示所属分组。
- **新增漫画分组管理页 `/comics/groups`**（管理员）：新建分组、勾选漫画批量移动、删除分组，与 `/novels/groups` 操作一致。
- **分组下拉更稳定**：选项带数量 `分组名（N）`，顺序固定（默认分组在前，其余按创建时间），不再随阅读记录跳动。
- **分组操作防误触**：「隐藏分组 / 取消隐藏」按钮只在进入具体分组后才出现，总览页不再渲染；「显示隐藏内容」并入分组下拉，作为下拉的最后一项。
- **书架列数改为外观设置**：原漫画书架工具条上的列数切换移入「我的 → 外观 → 书架列数」，可选 `自动 / 2 列 / 3 列`，对漫画与小说书架同时生效，仅窄屏（< 768px）生效，桌面保持自适应列数。

## Docker 部署

Docker Hub 镜像：[`dddinmx/mangadock:v2.8.0`](https://hub.docker.com/r/dddinmx/mangadock/tags)。`v2.8.0` 和 `latest` 同时提供 `linux/amd64` 与 `linux/arm64`。
