...
- 登录失败锁定改为数据库存储（原子 UPSERT），多进程 worker 下计数一致、重启不丢失；漫画库映射写入文件锁原子化；章节完整性判定更严格，杜绝生成 0 页空 CBZ/PDF。
- 番茄任务轮询增加总超时上限；统计页排名按用户分组权限过滤；前端封面加载重试防线与 fetch 超时保护。
...
- **分组**：书架页工具条的分组下拉用于筛选；分组的新建、批量移动与删除在「管理分组」页（漫画 `/comics/groups`、小说 `/novels/groups`，仅管理员）。
- **隐藏**：可以把单本漫画标记为隐藏，也可以隐藏整个分组。总览页不显示「隐藏分组」按钮，进入该分组后才会出现；被隐藏的内容需要在下拉里选「显示隐藏内容」才会列出。
- **列数**：在「我的 → 外观 → 书架列数」选择 `自动 / 2 列 / 3 列`，仅窄屏生效。
- **下载**：在「下载漫画」页粘贴支持的站点链接或书籍 ID，选择格式（CBZ / PDF）与目标分组后排队执行；「检查更新」用于补下已收藏作品的新章节。
