...
(function (global) {
    ...
    function isSafeNextPath(target) {
        if (typeof target !== 'string' || !target.startsWith('/') || target.startsWith('//') || target.indexOf('\\') !== -1) {
            return false;
        }
        try {
    ...
    function finishSuccess(form, result) {
        ...
        if (redirect && isSafeNextPath(redirect)) {
        ...
        scope.addEventListener('submit', function (event) {
            ...
            apiV1(requestSpec.method, requestSpec.path, requestSpec.body, { csrfToken: requestSpec.csrfToken })
                .then(function (result) {
                    ...
                    if (requestSpec.useDataNext && result.data && result.data.next && isSafeNextPath(result.data.next)) {
                        window.location.href = result.data.next;
                        return;
                    }
                    if (requestSpec.redirectOnSuccess && isSafeNextPath(requestSpec.redirectOnSuccess)) {
                        window.location.href = requestSpec.redirectOnSuccess;
                        return;
                    }
                    if (result.data && result.data.next && form.getAttribute('data-api-follow-next') === '1' && isSafeNextPath(result.data.next)) {
                        window.location.href = result.data.next;
                        return;
                    }
